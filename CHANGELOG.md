...
### Added
- Push-to-Talk and Push-to-Mute hotkeys — configurable key bindings with system-wide support (Tauri global shortcuts) and browser fallback, release delay (0–1000ms), coexistence of PTT+PTM with mute-wins priority, mute button auto-disabled when PTT/PTM active, dynamic keyboard shortcuts dialog entries
...
- Complete GDPR data export — export now includes direct messages, blocked users, reactions, file attachment metadata, sessions, E2EE device registrations, key backup metadata, and audit log entries; export manifest bumped to v1.1 (#289)
...
- Guild discovery frontend UI backfill — added dedicated discovery cards/listing, search and sort controls, and discoverable guild settings controls in the client experience (#271)
...
- Browser mode now stores refresh tokens as HttpOnly cookies instead of localStorage, preventing XSS token theft; access tokens are kept in memory only and restored via cookie-based refresh on page reload; Tauri clients continue using JSON body for backward compatibility (#288)
