...
export async function loadChannelsForGuild(guildId: string): Promise<void> {
  ...
  try {
    ...
    // Subscribe to all text channels in parallel
    const textChans = channelsState.channels.filter((c) => c.channel_type === "text");
    await Promise.all(
      textChans.map((channel) =>
        subscribeChannel(channel.id)
          .then(() => console.log(`[Channels] Subscribed to channel ${channel.name}`))
          .catch((err) => console.warn(`Failed to subscribe to channel ${channel.id}:`, err))
      )
    );
  } catch (err) {
    const error = err instanceof Error ? err.message : String(err);
