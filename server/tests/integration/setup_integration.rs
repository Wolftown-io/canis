...
//!
//! Run with: `cargo test --test integration setup_integration`
...
async fn test_concurrent_registrations_only_one_gets_admin() {
    ...
    for i in 0..num_concurrent {
        ...
        let handle = tokio::spawn(async move {
            ...
            let username = format!("concurrent_user_{i}_{test_id}");
    ...
    for result in results {
        ...
        if granted_admin {
            ...
            println!("    ✓ User {username} was NOT granted admin");
        }
    }
...
async fn test_concurrent_setup_completion_only_one_succeeds() {
    ...
    for result in results {
        ...
        if success {
            ...
            println!("    ✓ Task {task_id} successfully completed setup");
        } else {
            println!("    ✓ Task {task_id} found setup already complete");
        }
    }
    ...
    assert_eq!(
        success_count, 1,
        "Exactly one task should complete setup, got {success_count}"
    );
