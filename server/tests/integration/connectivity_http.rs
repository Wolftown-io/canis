...
async fn test_summary_with_data() {
    ...
    let channel_id = super::helpers::create_channel(
        &app.pool,
        super::helpers::create_guild(&app.pool, user_id).await,
        "voice-test",
    )
    .await;
...
async fn test_session_detail() {
    ...
    let token = generate_access_token(&app.config, user_id);
    let guild_id = super::helpers::create_guild(&app.pool, user_id).await;
    let channel_id = super::helpers::create_channel(&app.pool, guild_id, "voice-detail").await;
...
async fn test_session_rls_isolation() {
    ...
    let guild_id = super::helpers::create_guild(&app.pool, user_a).await;
