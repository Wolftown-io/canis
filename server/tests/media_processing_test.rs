...
//!
//! Run with: `cargo test --test media_processing_test -- --nocapture`
...
async fn test_image_upload_generates_metadata() {
    let (app, _bucket) = helpers::fresh_test_app_with_s3().await;
    let (user_id, _) = create_test_user(&app.pool).await;
    let token = generate_access_token(&app.config, user_id);
    ...
    let channel_id = helpers::create_channel(&app.pool, guild_id, "media-test").await;
...
async fn test_non_image_upload_no_metadata() {
    ...
    let perms = GuildPermissions::VIEW_CHANNEL | GuildPermissions::SEND_MESSAGES;
    let guild_id = helpers::create_guild_with_default_role(&app.pool, user_id, perms).await;
    let channel_id = helpers::create_channel(&app.pool, guild_id, "media-test-txt").await;
...
async fn test_variant_download_returns_webp() {
    ...
    let perms = GuildPermissions::VIEW_CHANNEL | GuildPermissions::SEND_MESSAGES;
    let guild_id = helpers::create_guild_with_default_role(&app.pool, user_id, perms).await;
    let channel_id = helpers::create_channel(&app.pool, guild_id, "media-test-dl").await;
...
async fn test_variant_fallback_to_original() {
    ...
    let perms = GuildPermissions::VIEW_CHANNEL | GuildPermissions::SEND_MESSAGES;
    let guild_id = helpers::create_guild_with_default_role(&app.pool, user_id, perms).await;
    let channel_id = helpers::create_channel(&app.pool, guild_id, "media-test-fb").await;
