...
//! and leave behavior.
//!
//! Run with: `cargo test --test dm_http_test -- --nocapture`
...
/// Create a TestApp with a fresh Redis client (not shared via OnceCell).
///
/// The shared `OnceCell<fred::clients::Client>` becomes non-functional when
/// the first test's tokio runtime drops. DM creation requires Redis (block
/// cache check), so we create a fresh client per test to avoid this.
async fn dm_test_app() -> TestApp {
